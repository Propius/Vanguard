...
CREATE OR REPLACE VIEW combined_sales_summary_view
AS
SELECT ROW_NUMBER() OVER () AS id,
       ...
       total_sales,
       type
FROM (SELECT CONCAT(DATE(date_of_sale), '-H', HOUR(date_of_sale)) AS period_of_sale,
             NULL                                                 AS game_no,
      ...
      SELECT DATE_FORMAT(date_of_sale, '%Y') AS period_of_sale,
             ...
             'YEARLY'                        AS type
      FROM game_sales
      GROUP BY DATE_FORMAT(date_of_sale, '%Y'), game_no) AS combined_sales_summary
